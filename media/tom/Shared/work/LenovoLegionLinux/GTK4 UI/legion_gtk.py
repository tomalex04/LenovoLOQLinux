...
def try_read_sysfs(filename):
    """Try to read a string from a sysfs file, returns None on failure."""
    try:
        path = filename if filename.startswith("/") else os.path.join(SYSFS_BASE, filename)
...
def try_write_sysfs(filename, value):
    """Try to write a value to a sysfs file directly (without pkexec), returns True if successful."""
    try:
        path = filename if filename.startswith("/") else os.path.join(SYSFS_BASE, filename)
...
class FanCurveWidget(Gtk.DrawingArea):
    ...
    Each point is [cpu_temp, gpu_temp, speed] matching the kernel's
    fancurve sysfs format: 'cpu_max_temp:gpu_max_temp:speed1'.
    Verified format from real hardware:
      75:60:0 80:70:30 90:80:34 97:80:37 ...
    """
    def __init__(self):
        super().__init__()
        self.set_hexpand(True); self.set_vexpand(True); self.set_size_request(600, 400)
        self.set_draw_func(self.on_draw)
        # Default points from verified hardware balanced mode
        self.points = [
            [75, 60, 0], [80, 70, 30], [90, 80, 34], [97, 80, 37],
            [97, 80, 37], [97, 80, 37], [97, 80, 37], [97, 80, 37],
            [99, 94, 37], [100, 20, 37]
        ]
        self.drag_idx = -1
    ...
    def on_draw(self, area, cr, width, height):
        ...
        # Draw control point circles (cyan like Vantage)
        for i, p in enumerate(self.points):
            x, y = ml + (i * w / 9), mt + h - (p[2] * h / 100)
            cr.arc(x, y, 7, 0, 2 * math.pi)
            if i == self.drag_idx: cr.set_source_rgb(1, 0.2, 0.2)
            else: cr.set_source_rgb(0.2, 0.7, 1)
            cr.fill()
        # Vertical dashed lines from each point down
    ...
    def on_drag_update(self, g, dx, dy):
        ...
        if not ok: return
        ns = max(0, min(100, int((h - (sy + dy - mt)) * 100 / h)))
        self.points[self.drag_idx][2] = ns
        # Enforce monotonically non-decreasing speed curve
...
class CustomSettingsWindow(Adw.Window):
    ...
    def __init__(self, parent, model):
        ...
        top_bar.append(Gtk.Label(label="Active preset:"))
        self.preset_combo = Gtk.DropDown(model=Gtk.StringList.new(["max", "min"]))
        top_bar.append(self.preset_combo)
        edit_btn = Gtk.Button(icon_name="document-edit-symbolic", css_classes=["flat", "circular"]); top_bar.append(edit_btn)
        del_btn = Gtk.Button(icon_name="user-trash-symbolic", css_classes=["flat", "circular"]); top_bar.append(del_btn)
        add_btn = Gtk.Button(label="+ Add", css_classes=["suggested-action", "pill"]); top_bar.append(add_btn)
        box.append(top_bar)
        ...
        bottom_bar.append(Gtk.Label(label="Load:"))
        self.load_combo = Gtk.DropDown(model=Gtk.StringList.new(["max", "min"]))
        bottom_bar.append(self.load_combo)
    ...
    def load_profiles(self):
        ...
        # Default profile with verified hardware balanced-mode values
        return {"Default": {
            "pl1": 65, "pl2": 80, "cross_load": 55, "peak": 20,
            "cpu_temp": 95, "pl2_duration_idx": 5,
            "dyn_boost": 15, "ctgp": 80, "gpu_temp": 87,
            "gpu_cpu_boost_idx": 0,
            "fan": [[75,60,0],[80,70,30],[90,80,34],[97,80,37],[97,80,37],[97,80,37],[97,80,37],[97,80,37],[99,94,37],[100,20,37]]
        }}
    ...
    def on_read_hw(self):
        ...
        # CPU Temperature Limit - no dedicated sysfs node yet, try gracefully
        cpu_temp_val = try_read_sysfs("cpu_temperature_limit")
        if cpu_temp_val is not None:
            try: self.cpu_temp.set_value(int(cpu_temp_val))
            except: pass

        # Short Term Power Limit Duration (tau) - no sysfs node yet, try gracefully
        tau_val = try_read_sysfs("cpu_l1_tau")
        if tau_val is not None:
            try:
                tau_items = [20, 24, 28, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160]
                tau_int = int(tau_val)
                if tau_int in tau_items:
                    self.pl2_duration.set_selected(tau_items.index(tau_int))
            except: pass

        # GPU to CPU Dynamic Boost - no sysfs node yet, try gracefully
        gpu_cpu_val = try_read_sysfs("gpu_cpu_dynamic_boost")
        if gpu_cpu_val is not None:
            try:
                boost_items = [0, 5, 10, 15]
                boost_int = int(gpu_cpu_val)
                if boost_int in boost_items:
                    self.gpu_cpu_boost.set_selected(boost_items.index(boost_int))
            except: pass

        # Read fan curve directly from sysfs
        try:
            content = try_read_sysfs(os.path.join(SYSFS_BASE, "fancurve"))
            if content:
                pts = []
    ...
    def _do_save(self, close):
        ...
        p = {
            ...
            "pl2_duration_idx": self.pl2_duration.get_selected(),
            ...
            "gpu_cpu_boost_idx": self.gpu_cpu_boost.get_selected(),
            "fan": self.graph.points
        }
        self.profiles[self.current_profile_name] = p
        with open(PROFILES_FILE, "w") as f: json.dump(self.profiles, f)

        # Build a single compound command for pkexec
        cmds = []
        slider_map = [
        ...
        # CPU Temperature Limit (try to write if sysfs node exists)
        if try_write_sysfs("cpu_temperature_limit", int(self.cpu_temp.get_value())):
            cmds.append(f"echo {int(self.cpu_temp.get_value())} > {SYSFS_BASE}/cpu_temperature_limit")

        # Short Term Power Limit Duration / tau (try to write if sysfs node exists)
        tau_items = [20, 24, 28, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160]
        tau_val = tau_items[self.pl2_duration.get_selected()]
        if try_write_sysfs("cpu_l1_tau", tau_val):
            cmds.append(f"echo {tau_val} > {SYSFS_BASE}/cpu_l1_tau")

        # GPU to CPU Dynamic Boost (try to write if sysfs node exists)
        gpu_cpu_items = [0, 5, 10, 15]
        gpu_cpu_val = gpu_cpu_items[self.gpu_cpu_boost.get_selected()]
        if try_write_sysfs("gpu_cpu_dynamic_boost", gpu_cpu_val):
            cmds.append(f"echo {gpu_cpu_val} > {SYSFS_BASE}/gpu_cpu_dynamic_boost")

        # Dynamic Boost dropdown
...
class LegionApp(Adw.Application):
    ...
    def update(self):
        try:
            self.cpu.set_subtitle(f"{int(self.m.cpu_temp.get()/1000)}\xb0C | {self.m.fan1_rpm.get()} RPM")
            self.gpu.set_subtitle(f"{int(self.m.gpu_temp.get()/1000)}\xb0C | {self.m.fan2_rpm.get()} RPM")
        except: pass
        return True
